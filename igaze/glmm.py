...
CONTINUOUS_OUTCOMES = [
    "mean_fixation_duration", "total_fixation_time", "fixation_rate",
    "mean_saccade_duration", "total_saccade_time", "mean_amplitude", "saccade_rate",
]
...
def _load_eye_metric_summaries(config_path: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return _load_summary("summary_csv", extract_fixations_from_config), _load_summary(
        "saccades_summary_csv",
        extract_saccades_from_config,
    )
...
def _normalize_model_df(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    for target, source in [("llm_provider", "AI"), ("prompt_type", "Prompt")]:
...
def _fit_mixedlm(df: pd.DataFrame, outcome: str):
    with warnings.catch_warnings():
        ...
        return smf.mixedlm(_fixed_effects_formula(outcome, df), df, groups=df["subject_id"]).fit()
...
def _fit_poisson_glmm(df: pd.DataFrame, outcome: str):
    with warnings.catch_warnings():
        ...
        return PoissonBayesMixedGLM.from_formula(
            _fixed_effects_formula(outcome, df),
...
def _extract_mixedlm_rows(mixed_results: dict[str, object]) -> list[dict[str, object]]:
    rows = []
    for outcome, model in mixed_results.items():
        conf_int = model.conf_int()
        for term in model.params.index:
            interval = conf_int.loc[term] if hasattr(conf_int, "loc") else conf_int[term]
            rows.append(
                {
                    "outcome": outcome,
                    "term": term,
                    ...
                    "ci_lower": interval.iloc[0] if hasattr(interval, "iloc") else interval[0],
                    "ci_upper": interval.iloc[1] if hasattr(interval, "iloc") else interval[1],
                },
            )
    return rows


def _extract_poisson_glmm_rows(count_results: dict[str, object]) -> list[dict[str, object]]:
    rows = []
    z_value = 1.96
    for outcome, model in count_results.items():
        for term, coef, posterior_sd in zip(model.model.exog_names, model.fe_mean, model.fe_sd):
            ci_lower = coef - z_value * posterior_sd
            ci_upper = coef + z_value * posterior_sd
            rows.append(
                {
                    "outcome": outcome,
                    "term": term,
                    ...
                    "ci_upper": ci_upper,
                    "rate_ratio": exp(coef),
                    "rate_ratio_ci_lower": exp(ci_lower),
                    "rate_ratio_ci_upper": exp(ci_upper),
                },
            )
    return rows
...
def run_glmm_models(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for col in COUNT_OUTCOMES + CONTINUOUS_OUTCOMES:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")
    df = df.dropna(subset=["subject_id", "llm_provider", "prompt_type"]).copy()
    ...
    mixed_rows = _extract_mixedlm_rows(mixed)
    count_rows = _extract_poisson_glmm_rows(count)
