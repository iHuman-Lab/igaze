...
def load_eyetracking_config(config_path: Path) -> tuple[Path, dict]:
...
def _save_frame(project_root: Path, et_cfg: dict, key: str, frame: pd.DataFrame) -> None:
    # Use the default output config structure
    default_outputs = {
...
def _subject_fixation_frames(subject_record: SubjectRecord, et_cfg: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
...
if __name__ == "__main__":
    print("No subjects defined in config. Nothing to process.")
